...
def index():
    if request.method == 'POST':
        ...
        for macroprocesso in macroprocessos:
            ...
            aba_nome = macroprocesso[:31]

            # Agrupar os dados por situação e operador de identificação
            agrupado = df_macroprocesso.groupby(
                ['Situação', 'Nome operador (identificação)', 'Nome operador (avaliação)']).size().reset_index(
                name='Qtd. de Docs')
            ...
            colunas_desejadas = ['Macroprocesso', 'Situação', 'Nome operador (identificação)',
                                 'Nome operador (avaliação)',
                                 'Qtd. de Docs', 'Status']
        ...
        server_destination = 'C:/Users/Patrick Ribeiro/Documents/dev/Rivic/Pendencias/files'  # Substitua pelo caminho absoluto no servidor
        ...
        return send_file('Relatorio_Macroprocessos.xlsx', as_attachment=True)
